...
const app = express();
const os = require('os'); // Para obter o IP
        ...
        if (firstAccount) {
            ...
            for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
                try {
                    ...
                    await page.goto('https://myaccount.mercadolivre.com.br/shipping/inbounds-v2?status=working', { waitUntil: 'networkidle2' });
                    console.log(`[${identifier}] Página de agendamentos aberta com cookies.`);

                    console.log(`[${identifier}] Aguardando 10 segundos para visualização...`); // Alterado o log
                    await new Promise(resolve => setTimeout(resolve, 10000)); // Espera 10 segundos
                    console.log(`[${identifier}] Visualização concluída.`); // Alterado o log
        ...
        for (let i = 1; i < accounts.length; i++) {
            ...
            for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
                try {
                    ...
                    console.log(`[${identifier}] Extraindo dados da tabela...`);
                    const tableSelector = "#app-root-wrapper > section > div:nth-child(5) > table";
                    const agendamentosTable = await extractTableData(page, tableSelector);

                    if (agendamentosTable && agendamentosTable.length > 0) {
                        console.log(`[${identifier}] Agendamentos com Data Reservada:`);
                        agendamentosTable.forEach(agendamento => {
                            console.log(`[${identifier}] Envio ID: ${agendamento.envioId}`);
                            console.log(`[${identifier}]   Unidades: ${agendamento.unidades}`);
                            console.log(`[${identifier}]   Data Reservada: ${agendamento.dataReservada}`);
                            console.log(`[${identifier}]   Custo Aplicado: ${agendamento.custoAplicado}`);
                            console.log(`[${identifier}]   Status: ${agendamento.status}`);
                            console.log(`[${identifier}] ------------------------------------`);
                        });
                    } else {
                        console.log(`[${identifier}] Nenhuma agendamento com Data Reservada encontrado.`);
                    }
            ...
            // Pequeno atraso antes de iniciar o próximo navegador (após o sucesso ou falha final de uma conta)
...
app.get('/agendamentos', async (req, res) => {
    ...
    try {
        ...
        console.log(`[API][${identifier}] Recarregando página de agendamentos para obter dados atualizados...`); // <-- NOVO LOG
        await page.goto('https://myaccount.mercadolivre.com.br/shipping/inbounds-v2?status=working', { waitUntil: 'networkidle2' });
